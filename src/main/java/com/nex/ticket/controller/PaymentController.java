...
public class PaymentController {
    ...
    public ResponseEntity<List<ChargeDto>> getAllCharges() {
        ...
        List<ChargeDto> dtos = records.stream()
                .map(r -> new ChargeDto(r.getId(), r.getAmount(), r.getCurrency(), r.getStatus(), r.getSourceId(), r.getCreatedAt(), null))
    ...
    public ResponseEntity<ChargeDto> updateChargeStatus(
        ...
        ChargeDto dto = new ChargeDto(record.getId(), record.getAmount(), record.getCurrency(), record.getStatus(), record.getSourceId(), record.getCreatedAt(), null);
    ...
    private ResponseEntity<Map<String, Object>> createOmiseCharge(int amountBaht, SourceType type) {
        try {
            ...
            String mockId = ChargeStore.addCharge("mock-source-" + type.toString(), (long) amountBaht * 100, "THB", null);
            Map<String, Object> response = new LinkedHashMap<>();
